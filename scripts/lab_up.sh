...
if ! command -v docker >/dev/null 2>&1; then
  echo "[!] Docker is not installed - and the student labs do NOT need it."
  echo "    Modules 1-5 run directly against the shared Ollama + Wazuh (no Docker)."
  echo "    Docker is only for the OPTIONAL extras this script starts: your own"
  echo "    at-home victims/attacker and the offline mock. Install (Ubuntu):"
